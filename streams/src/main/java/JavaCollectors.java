...
public class JavaCollectors {
    public static void main(String[] args) {
        ...
        Map<Integer, Long> nameCount = DishData.dishList().stream().filter(each -> each.getCalories() > 500).collect(Collectors.groupingBy(Dish::getCalories, Collectors.counting()));
        System.out.println(nameCount);
        System.out.println("*******************");
        Map<String, Double> avgCal = DishData.dishList().stream().collect(Collectors.groupingBy(Dish::getName, Collectors.averagingInt(Dish::getCalories)));
